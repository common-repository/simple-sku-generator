     ...
     public function __construct(){
	  ...
	  public function check_if_woo_install(){
	   ...
	   public function check_versions(){
	    ...
	    require_once(ABSPATH . 'wp-admin/includes/plugin.php');
	    deactivate_plugins( plugin_basename( __FILE__ ));
	    wp_die( __('Simple SKU Generator is disabled.<br>Simple SKU Generator requires a minimum of WooCommerce v4.8.0.<br><a href="'.$url.'">Return to the Plugins section</a>'));
	    }
	    }
       ...
       function hp_sku_var_generator(){
	    ...
	    $w_cat = filter_input(INPUT_POST, 'w_cat', FILTER_SANITIZE_STRING, FILTER_REQUIRE_ARRAY);
		...
		if($_POST){
	    $w_cat = isset($_POST['w_cat']) ? $_POST['w_cat'] : '';
		$args = array('post_type' => array('product'), 'post_status' => 'publish', 'product_cat' => ''.$w_cat.'', 'posts_per_page' => -1);
		...
		global $wpdb;
		$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}woo_sku1 (w_id, w_title, w_cat, w_item) VALUES (%s, %s, %s, %s)", $post->ID, $post->post_title, $w_cat, $w_item));
		}
		}
        ...
        echo '<div class="w_cat">';
        echo '<label for="w_cat" style="font-size:14px;">Select a category: </label>';
        echo '<select name="w_cat" required id="w_cat" data-required="Please select a category." require class="at-required">';
	  ...
	  function hp_sku_var_display(){
	   ...
	   global $wpdb;
	   $try0 = $wpdb->prepare("SELECT s1.w_id, s1.w_title, s1.w_cat, s1.w_item, s2.w_day, s2.w_month, s2.w_year, s2.w_item FROM {$wpdb->prefix}woo_sku1 AS s1 INNER JOIN {$wpdb->prefix}woo_sku2 AS s2 WHERE s1.id=s2.id LIMIT 1");
       $result0 = $wpdb->get_results($try0); 
	   foreach($result0 as $row0){
	   echo '<p style="font-size: 14px;"><strong> Category:<span style="margin: 0px 0px 0px 6px;">'.sanitize_text_field($row0->w_cat).'</span></strong> </p>';
	   }
	   $try1 = $wpdb->prepare("SELECT s1.w_id, s1.w_title, s1.w_cat, s1.w_item, s2.w_day, s2.w_month, s2.w_year, s2.w_item FROM {$wpdb->prefix}woo_sku1 AS s1 INNER JOIN {$wpdb->prefix}woo_sku2 AS s2 WHERE s1.id=s2.id");
	   ...
	   if($_POST['remove_sku']) {
	   global $wpdb;
	   $wpdb->get_results("DELETE {$wpdb->prefix}woo_sku1,{$wpdb->prefix}woo_sku2 FROM {$wpdb->prefix}woo_sku1 INNER JOIN {$wpdb->prefix}woo_sku2 WHERE {$wpdb->prefix}woo_sku1.id={$wpdb->prefix}woo_sku2.id");
	   } 
	   }
	   echo '<script type="text/javascript">function toggle(source) {
       checkboxes = document.getElementsByName("remove_sku[]"); 
	   ...
	   <thead>
	   <tr>
	   <th class="check-column" scope="col" style="border: 1px solid black;"><input type="checkbox" value="'.sanitize_text_field($row2->w_id).'" onClick="toggle(this)"></th>
	   ...
	   echo '<tr>
	   <td style="border: 1px solid black;" align="center"><input type="checkbox" name="remove_sku[]" value="'.sanitize_text_field($row2->w_id).'" ></td>
